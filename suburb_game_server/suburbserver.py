...
class User:
    ...
    def create_user(cls, name, password) -> Optional["User"]:
        if name in database.memory_users:
            return None
    ...
    def verify_token(self, token: str, refresh_token=True):
        ...
        if expiry is None:
            ...
            if (
                datetime.datetime.strptime(expiry, "%m/%d/%Y, %H:%M:%S")
                < datetime.datetime.now()
            ):
                return False
            else:
                if refresh_token:
                    new_expiration_time = datetime.datetime.now() + datetime.timedelta(
                        hours=1
                    )
                    self.token = token, new_expiration_time.strftime(
                        "%m/%d/%Y, %H:%M:%S"
                    )
                return True
...
def handle_request(dict):
    ...
    if intent == "search_items":
        results = difflib.get_close_matches(
            content, list(util.bases.keys()), n=5, cutoff=0.5
        )
        if content in util.kinds:
            results += [
                base_name
                for base_name in util.bases
                if content in alchemy.Item(base_name).kinds
            ]
        return json.dumps(results)
    if intent == "item_info":
        item_name = content
        if item_name not in util.bases:
            return json.dumps({})
        else:
            return json.dumps(alchemy.Item(item_name).get_dict(raw=True))
    if intent == "all_tiles":
        return json.dumps(
            {
                "tiles": [
                    tile_char
                    for tile_char in tiles.tiles
                    if not tiles.tiles[tile_char].debug
                ],
                "labels": {
                    tile.tile_char: tile.name
    ...
    if intent == "server_tiles":
        return json.dumps(
            {
                "server_tiles": tiles.server_tiles,
                "labels": {tile.tile_char: tile.name for tile in tiles.tiles.values()},
            }
        )
    if intent == "interests":
        return json.dumps(sorted(spawnlists.get_interest_spawnlists()))
    if intent == "tile_spawnlists":
        return json.dumps(sorted(spawnlists.get_tile_spawnlists()))
    if intent == "kinds":
    ...
    if intent == "all_session_characters":
        ...
        for session_name in user.sessions:
            ...
            if session_player is None:
                out_dict[session_name] = None
            else:
                out_dict[session_name] = session_player.get_dict()
    ...
    if intent == "submit_item":
        ...
        verified_item_dict["display_name"] = (
            str(item_dict["display_name"]) if item_dict["display_name"] else None
        )
        verified_item_dict["power"] = int(item_dict["power"])
        verified_item_dict["size"] = int(item_dict["size"])
        if not item_dict["kinds"]:
            return "Your item needs at least one kind!"
        verified_item_dict["kinds"] = [
            kind_name for kind_name in item_dict["kinds"] if kind_name in util.kinds
        ]
        verified_item_dict["wearable"] = bool(item_dict["wearable"])
        if not item_dict["description"]:
            return "You need to give your item a description!"
        verified_item_dict["description"] = str(item_dict["description"])
        if "build" not in item_dict["cost"] or item_dict["cost"]["build"] != 0.5:
            return "fucker..."
        for state_name in item_dict["onhit_states"]:
            if state_name not in stateseffects.item_states:
                return f"Invalid state {state_name}"
        ...
        if not item_dict["secretadjectives"]:
            return (
                f"You need to put in inherited adjectives! Please include at least 4."
            )
        verified_item_dict["secretadjectives"] = [
            str(adjective) for adjective in item_dict["secretadjectives"]
        ]
        ...
        if item_dict["code"] is None:
            ...
            if not binaryoperations.is_valid_code(item_dict["code"]):
                return f"Invalid code {item_dict['code']}"
            elif item_dict["code"] in util.codes:
        ...
        if len(item_dict["interests"] + item_dict["tiles"]) == 0:
            return "Include at least one place for your item to spawn."
    ...
    if session_name in user.sessions and intent == "join_session":
        return "You are already in that session!"
    if session_name not in user.sessions:
        ...
        if intent == "create_session":
            ...
            if session is None:
                return f"The session `{session_name}` is already registered."
    ...
    session = sessions.Session(session_name)
    if session is None:
        return f"Session `{session_name}` no longer exists."
    if intent == "session_info":
        out = {}
        out["current_grist_types"] = session.current_grist_types
        return json.dumps(out)
    if intent == "create_character":
        if session.get_current_subplayer(user.name) is not None:
            return "Character was already created."
        ...
        try:
            ...
            for interest in interests:
                if interest not in spawnlists.get_interest_spawnlists():
        ...
        # create player
        new_player = sessions.Player.create_player(desired_name, username, session)
        ...
        land = sessions.Land.create(
            f"{new_player.id}{session.name}", session, new_player
        )
    ...
    match intent:
            ...
            if player.strife is None:
                if player.room.strife is not None:
                    ...
                    if griefer is None:
                        ...
                        return json.dumps(player.room.strife.get_dict())
                else:
                    return json.dumps({})
            else:
        ...
        case "carved_item_info":
            ...
            if not alchemy.does_instance_exist(dowel_name):
                print("dowel does not exist")
                return {}
        ...
        case "use_item":
            ...
            if not alchemy.does_instance_exist(instance_name):
                return False
            instance = alchemy.Instance(instance_name)
            if target_name is not None:
                target_instance = alchemy.Instance(target_name)
            else:
                target_instance = None
            return use_item(
                player, instance, action_name, target_instance, additional_data
            )
        case "interact_npc":
            npc_name = content["npc_name"]
            interaction_name = content["interaction_name"]
            additional_data = content["additional_data"]
            if npc_name not in player.room.npcs:
                return False
            if interaction_name not in npcs.npc_interactions:
                return False
            if interaction_name not in npcs.Npc(npc_name).interactions:
                return False
            return npcs.npc_interactions[interaction_name].use(
                player, npcs.Npc(npc_name), additional_data
            )
        case "computer":
        ...
        case "eject_from_strife_deck":
            instance_name = content["instance_name"]
            if player.wielding == instance_name:
                player.unwield()
        ...
        case "valid_use_targets":
            ...
            action_name = content["action_name"]
            instances_dict = {}
            valid_names = valid_use_targets(
                player, alchemy.Instance(instance_name), action_name
            )
        ...
        case "get_client_server_chains":
            server_client = {
                player_username: sessions.Player(player_username).client_player_name
                for player_username in session.starting_players
            }
            player_names = {
                player_username: sessions.Player(player_username).nickname
                for player_username in session.starting_players
            }
            no_server = []
            chained = []
            ...
            for player_username in session.starting_players:
                ...
                if player.server_player_name is None:
                    no_server.append(player_username)
            # find which players are in a chain and which aren't
            for player_username in session.starting_players:
                if player_username in chained:
                    continue
        ...
        case "submit_strife_action":
            ...
            if player.strife is None:
                print("strife is none")
                return json.dumps({})
            success = player.strife.get_griefer(player.name).submit_skill(
                skill_name, targets
            )
            if success:
                return json.dumps(player.strife.get_dict())
        ...
        case "collect_spoils":
            ...
            player.claim_spoils()
            return json.dumps(
                {"unclaimed_grist": unclaimed_grist, "unclaimed_rungs": unclaimed_rungs}
            )
        case "get_resulting_alchemy":
            ...
            if code_1 in util.codes and code_2 in util.codes:
                ...
                alchemized_item_name = alchemy.alchemize(
                    item_1.name, item_2.name, operation
                )
        ...
        case "prototype_targets":
            valid_targets = {
                ...
                for instance_name in player.room.instances
            }
            valid_targets.update(
                {
                    instance_name: alchemy.Instance(instance_name).get_dict()
                    for instance_name in player.sylladex
                }
            )
...
def map_data(player: "sessions.SubPlayer"):
    map_tiles, map_specials, room_instances, room_npcs, room_players, strife = (
        player.get_view()
    )
    return json.dumps(
        {
            "map": map_tiles,
            "specials": map_specials,
            "instances": room_instances,
            "npcs": room_npcs,
            "players": room_players,
            "strife": strife,
            "room_name": player.room.tile.name,
            "theme": player.overmap.theme,
        }
    )
...
def get_viewport(x: int, y: int, client: Optional[sessions.Player]) -> str:
    ...
    excursus_contents = {
        item.name: item.get_dict()
        for item in [
            alchemy.Item(item_name) for item_name in client.starting_session.excursus
        ]
    }
    atheneum = {
        instance.name: instance.get_dict()
        for instance in [
            alchemy.Instance(instance_name) for instance_name in client.atheneum
        ]
    }
    return json.dumps(
        {
            ...
            "instances": room_instances,
            "npcs": room_npcs,
            "room_name": room.tile.name,
            "players": room_players,
            "client_grist_cache": client_grist_cache,
            "client_available_phernalia": client_available_phernalia,
            "client_cache_limit": client.grist_cache_limit,
            "atheneum": atheneum,
            "excursus": excursus_contents,
            "theme": client.land.theme,
            "player_color": client.symbol_dict["color"],
        }
    )
...
def computer_shit(player: sessions.SubPlayer, content: dict, session: sessions.Session):
    ...
    match command:
        ...
        case "leech":
            ...
            if grist_type not in config.grists:
                return "fuck you"
            if grist_type in player.leeching:
                player.leeching.remove(grist_type)
            else:
                player.leeching.append(grist_type)
        case "connect":
            client_player_username = content["client_player_username"]
            if not sessions.does_player_exist(client_player_username):
                return "Player does not exist."
            client_player = sessions.Player(client_player_username)
            if client_player.server_player_name is not None:
                return "Client already has server."
            if player.client_player_name is not None:
                return "You already have a client."
            ...
            client_player.grist_cache["build"] += min(
                2 * (10 ** len(client_player.starting_session.connected)), 2000
            )
            return "Successfully connected."
        case "deploy_phernalia":
            if player.client_player is None:
                return "No client."
            x_coord = content["x"]
            y_coord = content["y"]
            viewport_x = content["viewport_x"]
        ...
        case "add_to_atheneum":
            ...
            viewport_y = content["viewport_y"]
            instance_name = content["instance_name"]
            target_room = player.client_player.land.housemap.find_room(
                viewport_x, viewport_y
            )
            if instance_name not in target_room.instances:
                return False
        ...
        case "recycle":
            if player.client_player is None:
                return "No client."
            instance_name = content["instance_name"]
            if instance_name not in player.client_player.atheneum:
                return False
            instance = alchemy.Instance(instance_name)
        ...
        case "get_alchemiter_location":
            if player.client_player is None:
                return "No client."
            alchemiter_location = (
                player.client_player.land.housemap.get_alchemiter_location()
            )
        ...
        case "server_alchemy":
            ...
            if alchemiter_loc is None:
                return "No alchemiter."
            code = content["code"]
...
def console_commands(player: sessions.SubPlayer, content: str):
    ...
    match command:
        ...
        case "homestuck":
            player.goto_room(
                player.land.housemap.random_valid_room(config.starting_tiles)
            )
        case "tp":
...
def use_item(
    ...
    action_name,
    target_instance: Optional[alchemy.Instance] = None,
    ...
    if (
        ...
        and target_instance.name not in player.room.instances
        and target_instance.name not in player.sylladex
    ):
        print("not in room")
        return False
    if action_name not in instance.item.use:
        return False
    match action_name:
        case "add_card":
            if player.empty_cards >= 10:
                return False
        ...
        case "install_sburb":
            if target_instance is None:
                return False
            if "computer" not in target_instance.item.use:
                return False
            if "Sburb" not in target_instance.computer_data["installed_programs"]:
                target_instance.computer_data["installed_programs"].append("Sburb")
            return True
        case "install_gristtorrent":
            ...
            if (
                "gristTorrent"
                not in target_instance.computer_data["installed_programs"]
            ):
                target_instance.computer_data["installed_programs"].append(
                    "gristTorrent"
                )
            return True
        case "combine_card":
            if target_instance is None:
                return False
            if target_instance.name not in player.sylladex:
                return False
            if target_instance.punched_code == "" or instance.punched_code == "":
                return False
            if (
                target_instance.item.name != "punched card"
                or instance.item.name != "punched card"
            ):
                return False
            # if both items are real and not just bullshit
            new_code = ""
            if (
                target_instance.punched_code in util.codes
                and instance.punched_code in util.codes
            ):
                ...
                alchemized_item_name = alchemy.alchemize(
                    currently_punched_item.name, additional_item.name, "&&"
                )
                alchemized_item = alchemy.Item(alchemized_item_name)
                new_code = alchemized_item.code
            else:
                # otherwise the code is just bullshit
                new_code = binaryoperations.codeor(
                    target_instance.punched_code, instance.punched_code
                )
            ...
            if not player.consume_instance(target_instance.name):
                return False
        ...
        case "uncombine_card":
            if instance.combined == []:
                print("not combined")
                return False
            if not player.consume_instance(instance.name):
                print("couldnt consume")
                return False
        ...
        case "enter":
            if not player.entered:
                ...
                player.land.housemap.populate_with_underlings(
                    "imp", 4, random.randint(40, 60), 1, 7
                )
                player.land.housemap.populate_with_underlings(
                    "ogre", 1, random.randint(1, 4), 1, 2
                )
                ...
                if not player.prototyped_before_entry:
                    player.session.prototypes.append(None)
                return True
            else:
                return False
        case "collect":
        ...
        case "insert_card":
            ...
            player.empty_cards -= 1
            if (
                target_instance.item.name != "captchalogue card"
                and target_instance.item.name != "punched card"
            ):
        ...
        case "punch_card":
            ...
            if len(code_to_punch) != 8:
                print("invalid code")
                return False
            for char in code_to_punch:
                if char not in binaryoperations.bintable:
                    print("invalid code")
                    return False
            if code_to_punch == "00000000":
                return True  # no holes would be made
            ...
            # if both items are real and not just bullshit
            if (
                inserted_instance.punched_code in util.codes
                and code_to_punch in util.codes
            ):
                currently_punched_item = alchemy.Item(
                    util.codes[inserted_instance.punched_code]
                )
        ...
        case "insert_dowel":
            if instance.inserted != "":
                print("something is already inserted")
                return False
            if target_instance is None:
                print("no target")
                return False
            if target_instance.item.name != "cruxite dowel":
                print("not a dowel")
                return False
            if target_instance.carved != "00000000":
                print("dowel already carved")
                return False
            if target_instance.name in player.sylladex:
                if not player.consume_instance(target_instance.name):
                    print("couldn't consume")
                    return False
            else:
                if target_instance.name not in player.room.instances:
                    print("couldn't find dowel in room")
                    return False
        ...
        case "lathe":
            ...
            if target_instance is None:
                print("no target")
                return False
            if target_instance.item.name != "punched card":
                print("not punched card")
                return False
            inserted_instance = alchemy.Instance(instance.inserted)
...
def valid_use_targets(
    player: sessions.Player, instance: alchemy.Instance, action_name
) -> list[str]:
    valid_target_names = []
    match action_name:
        case "install_sburb":
            ...
            def filter_func(name):
                if "computer" not in alchemy.Instance(name).item.use:
                    return False
                return True
        ...
        case "insert_card":
            ...
            def filter_func(name):
                if name not in player.sylladex:
                    return False
                return True
        ...
        case "punch_card":
            ...
            def filter_func(name):
                if name not in player.sylladex:
                    return False
                if alchemy.Instance(name).item.forbiddencode:
                    return False
                return True
        ...
        case "combine_card":
            ...
            def filter_func(name):
                ...
                if name not in player.sylladex:
                    return False
                if alchemy.Instance(name).punched_code == "":
                    return False
                return True
        ...
        case "insert_dowel":
            ...
            def filter_func(name):
                ...
                if filter_instance.item.name != "cruxite dowel":
                    return False
                if filter_instance.carved != "00000000":
                    return False
                return True
        ...
        case "lathe":
            ...
            def filter_func(name):
                if name not in player.sylladex:
                    return False
                filter_instance = alchemy.Instance(name)
                if filter_instance.item.name != "punched card":
                    return False
                return True
