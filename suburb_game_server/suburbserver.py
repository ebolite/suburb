...
def handle_request(dict):
    ...
    if intent == "all_tiles":
        return json.dumps({"tiles": [tile_char for tile_char in tiles.tiles if not tiles.tiles[tile_char].debug], "labels": {tile.tile_char:tile.name for tile in tiles.tiles.values() if not tile.debug}})
