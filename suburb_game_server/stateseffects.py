...
class DouseState(OneTimeState):
    def on_apply(self, griefer: "strife.Griefer"):
        ...
        griefer.strife.log(
            f"{griefer.nickname} was DOUSED of {cured_state.name.upper()}!"
        )
...
class LeechState(OneTimeState):
    def on_apply(self, griefer: "strife.Griefer"):
        ...
        griefer.strife.log(
            f"{amount_to_leech} build and {second_grist} grist was LEECHED!"
        )
        return super().on_apply(griefer)
...
stun.tooltip = (
    "Significantly reduces VIM. Can only be applied once per target per strife."
)
...
freeze.tooltip = (
    "Deals significant damage. Can only be applied once per target per strife."
)
...
class ClassPassive(State):
    def __init__(
        self, name, aspect: "skills.Aspect", class_name: str, required_rung: int
    ):
        if class_name not in class_passives:
            class_passives[class_name] = {}
        if aspect.name not in class_passives[class_name]:
            class_passives[class_name][aspect.name] = {}
...
class DemiseState(ClassPassive):
    def new_turn(self, griefer: "strife.Griefer"):
        ...
        griefer.strife.log(
            f"Everyone's {self.aspect.name.upper()} decreased by {-adjust_reply}!"
        )
...
for _, aspect in skills.aspects.items():
    ...
    bodystate.tooltip = (
        f"Gain additional actions each turn based on your {aspect.name}."
    )
    ...
    demisestate.tooltip = (
        f"The {aspect.name.upper()} of everyone in the strife decreases each turn."
    )
