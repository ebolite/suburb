...
class Griefer:
    ...
    def can_pay_costs(self, costs: dict[str, int]) -> bool:
        for vial_name in costs:
            if self.get_usable_vial(vial_name) < costs[vial_name]:
                return False
        return True
    ...
    def can_use_skill(self, skill: Skill) -> bool:
        if not skill.usable:
            return False
        if not self.can_pay_costs(skill.costs):
            return False
        if self.get_skill_cooldown(skill) > 0:
            return False
        if self.available_actions < skill.action_cost:
            return False
        return True
    ...
    def known_skills_list(self) -> list[Skill]:
        return [
            Skill(skill_name, self.known_skills[skill_name])
...
class Strife:
    ...
    def make_griefer_sprites(self):
        ...
        for sprite in red_sprites + blue_sprites:
            if scale > 0.66 and sprite.get_height() > 800:
                ...
                scale = 0.75
        if len(red_sprites + blue_sprites) > 4 and scale > 0.75:
            scale = 0.75
        if len(red_sprites + blue_sprites) > 9 and scale > 0.66:
            scale = 0.66
        for sprite in red_sprites + blue_sprites:
            sprite.scale = scale
    ...
    ) -> Union["render.Enemy", "render.PlayerGriefer"]:
    ...
    def get_skill_button_func(self, skill_name):
        def button_func():
            ...
            if not self.player_griefer.can_use_skill(skill):
                return
            if self.player_griefer.available_actions < skill.action_cost:
                return
            if skill_name != self.selected_skill_name:
                self.selected_targets = []
            self.selected_skill_name = skill_name
            self.hovered_skill_name = skill_name
            if (
                self.selected_skill is not None
                and len(self.selected_skill.valid_targets) == 1
            ):
    ...
    def make_skill_button(self, skill: Skill, x: int, y: int) -> "render.TextButton":
        if self.player_griefer.get_skill_cooldown(skill) > 0:
            ...
            button_text = (
                f"{'[I] ' if skill.action_cost <= 0 else ''}>{skill.name.upper()}"
            )
        skill_button = render.TextButton(
            x, y, 196, 32, button_text, self.get_skill_button_func(skill.name)
        )
    ...
    def make_hotkey_label(
        self, button: "render.TextButton", i, category_name, first_layer=True
    ):
        if i not in self.hotkeys:
            return
    ...
    def draw_scene(self):
        bar_br = 30
        self.canvas = render.SolidColor(
            0.5, 0.5, render.SCREEN_WIDTH, render.SCREEN_HEIGHT, self.theme.light
        )
        ...
        self.make_griefer_sprites()
        top_bar = render.SolidColor(
            0, -bar_br, render.SCREEN_WIDTH, 120 + bar_br, self.theme.light
        )
        ...
        for category_name in self.player_griefer.skill_categories:
            if category_name == "none":
                continue
            category_button = render.TextButton(
                4,
                y,
                196,
                32,
                f"{category_name.upper()}",
                self.get_category_button_func(category_name),
            )
        ...
        revert_button = render.TextButton(
            0.85, 0.2, 196, 32, ">REVERT", revert_button_func
        )
        revert_button.draw_condition = button_condition
        revert_button_label = render.Text(0.5, 1.2, "(escape)")
        revert_button_label.text_func = lambda *args: (
            "(escape)" if button_condition() else ""
        )
        revert_button_label.bind_to(revert_button)
        revert_button_label.fontsize = 12
        revert_button_label.color = self.theme.dark
        submit_button = render.TextButton(
            0.5, 0.78, 256, 48, ">READY", submit_button_func
        )
        ...
        submit_button_label.text_func = lambda *args: (
            "(space)" if button_condition() else ""
        )
        submit_button_label.bind_to(submit_button)
        submit_button_label.fontsize = 12
        submit_button_label.color = self.theme.dark
        self.strife_log_window = render.LogWindow(
            None, None, lines_to_display=5, log_list=self.strife_log
        )
        self.submitted_skills_window = render.LogWindow(
            None, None, lines_to_display=4, x=1080, width=300, log_list=[]
        )
        ...
        self.remaining_skills_text.text_func = (
            lambda *args: f"Actions left: {self.player_griefer.available_actions}"
        )
        self.remaining_skills_text.color = self.theme.dark
        self.remaining_skills_text.fontsize = 28

        def currently_selected_text_func():
            if self.player_griefer.ready:
                return "Waiting for others..."
            elif self.selected_skill is None:
                return ""
            else:
                return f"{self.selected_skill.name.upper()}? (target {self.selected_skill.num_targets - len(self.selected_targets)})"
    ...
    def make_skill_info_window(self):
        PADDING = 8
        BOX_HEIGHT = 120
        self.hovered_skill_info_box = render.SolidColor(
            0,
            render.SCREEN_HEIGHT - BOX_HEIGHT,
            render.SCREEN_WIDTH,
            BOX_HEIGHT,
            self.theme.light,
        )
    ...
    def make_skill_tooltip_display(self):
        ...
        fontsize = min(
            self.hovered_skill_info_box.h // len(joined_lines) - LINE_PADDING, 20
        )
        for i, line in enumerate(joined_lines):
            line_text = render.Text(
                PADDING, PADDING * 5 + (fontsize + LINE_PADDING) * i, line
            )
    ...
    def update(self):
        ...
        if pygame.mouse.get_pressed()[0] and self.layer_2_buttons:
            for button in self.layer_2_buttons:
                ...
                self.clear_next_layer_buttons()
        if (
            self.strife_log_window.is_mouseover()
            and self.strife_log_window.lines_to_display < 30
        ):
            self.strife_log_window.lines_to_display += 5
            self.strife_log_window.update_logs()
        elif (
            not self.strife_log_window.is_mouseover()
            and self.strife_log_window.lines_to_display > 5
        ):
            self.strife_log_window.lines_to_display -= 5
            self.strife_log_window.update_logs()
        self.update_submitted_skills()
    ...
    def keypress(self, event):
        if event.key == pygame.K_ESCAPE:
            if self.layer_2_buttons:
                self.clear_next_layer_buttons()
            elif self.selected_skill_name is not None:
    ...
    def delete(self):
        if self in render.ui_elements:
            render.ui_elements.remove(self)
    ...
    def submit_skill(self):
        reply = client.requestplusdic(
            intent="submit_strife_action",
            content={
    ...
    def update_vials(self):
        for vial_name in self.player_griefer.vials:
            ...
            if vial_name in self.vials:
                continue
            elif hidden and self.player_griefer.get_vial(
                vial_name
            ) == self.player_griefer.get_starting_vial(vial_name):
                continue
            else:
                self.make_vials()
    ...
    def make_vials(self):
        ...
        for vial_name in self.player_griefer.vials:
            hidden = config.vials[vial_name]["hidden"]
            if hidden and self.player_griefer.get_vial(
                vial_name
            ) == self.player_griefer.get_starting_vial(vial_name):
                continue
            vial_y += vial_y_increase
            new_vial = render.GrieferVial(
    ...
    def selected_skill(self) -> Optional[Skill]:
        if self.selected_skill_name is None:
            return None
    ...
    def hovered_skill(self) -> Optional[Skill]:
        if self.hovered_skill_name is None:
            return None
