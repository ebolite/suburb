    ...
    def make_symbol(self):
        ...
        def get_button_func(part, direction: str):
            def button_func():
                ...
                if self.symbol_dict["style_dict"][part] not in config.part_styles[part][new_item_name]:
                    self.symbol_dict["style_dict"][part] = random.choice(config.part_styles[part][new_item_name])
...
def display_item(instance: Instance, last_scene:Callable, modus:Optional[Modus] = None, flipped=False, strife=False): 
    ...
    if not flipped:
        ...
        for i, action_name in enumerate(instance.item.use):
            ...
            action_button = render.Button(x, y, path, pressed_path, instance.get_action_button_func(action_name, last_scene), theme=theme)
    ...
    if strife:
        if not instance.item.wearable or instance.name != player_data["worn_instance_name"]:
