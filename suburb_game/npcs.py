...
class Npc:
    ...
    def get_npc_interaction_button(self, interaction_name: str, last_scene: Callable):
        if interaction_name not in npc_interactions:
            return lambda *args: util.log("Unimplemented")
        else:
...
class NpcPrototype(NpcInteraction):
    def use(self, npc: Npc, last_scene: Callable):
        ...
        syl.update_deck()
        util.log(
            f"What will you prototype {npc.nickname if npc.nickname != 'kernelsprite' else f'the kernelsprite'} with?"
        )
        ...
        def get_button_func(target_instance: sylladex.Instance):
            def button_func():
                ...
                reply = self.request_npc_interaction(
                    npc, {"instance_name": target_instance.name}
                )
                util.log(reply)
                last_scene()
        ...
        for i, target_instance_name in enumerate(valid_instances.keys()):
            ...
            target_instance = sylladex.Instance(
